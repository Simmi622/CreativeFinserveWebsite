 ...
 * @license
 * 
 * Name: GBST - Shared - Radiogroup Input
 * Artifact ID: radiogroup-input
 * Version: 1.2.8
 * 
 */
 ...
 * @class
 */
view.RadiogroupView = Backbone.View.extend({
	 ...
	 * This may be called at any time after initialisation to update the visual state of the control.
	 * 
	 * NOTE: Ensure this view's element has been added to the DOM prior to calling render().
	 */
	,render: function(){
		...
		if (this.viewDirty){
			...
			var elements = this.elements = this.$el.find('fieldset');
			
			if (controlConfig.templateData.vertical) {
				elements.buttonsetv();
			} else {
				elements.buttonset();
			}
			...
			// Listen for change events from the range control.
			elements.bind("change", $.proxy(this.inputChange_handler, this));
	...
	,prepareContext: function(controlConfig) {
		...
		if (controlConfig.templateData.title) {
			context.templateData.title = I18n.t(controlConfig.templateData.title);
		}
		if(controlConfig.templateData.legend) {
			context.templateData.legend = I18n.t(controlConfig.templateData.legend);
		}
	...
	/**
	 * Refresh buttons value
	 */
	,refreshValue: function() {
		
		var currentValue = this.model.get(this.controlConfig.name);
		this.setControlValue(currentValue);
	}
	...
	,setControlValue: function(value){
		...
		// Ref: http://stackoverflow.com/questions/3759088/jquery-ui-radio-button-how-to-correctly-switch-checked-state
		var radio = this.$el.find(":radio");
		radio.val([controlValue]);
	...
	,inputChange_handler: function(e){
		 ...
		 * Set the value on the model.
		 */
		var propertyName = this.controlConfig.name;
		this.model.set(propertyName, finalValue);
